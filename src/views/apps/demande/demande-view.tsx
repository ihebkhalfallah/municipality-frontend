...
const DemandeView = () => {
  ...
  const fetchDemandes = useCallback(async () => {
    try {
      const response = await getDemandes(page + 1, rowsPerPage);
  ...
  const handleAcceptDemande = async (id: number) => {
    try {
      await updateDemande(id, { status: DEMANDE_STATUS.ACCEPTED });
      fetchDemandes();
  ...
  const handleAddComment = async () => {
    try {
      const newComment = await addComment(selectedDemande?.id || 0, comment);
      if (files.length > 0) {
  ...
  const handleUploadFiles = async () => {
    try {
      ...
      setAlert({ message: 'Error uploading files', severity: 'error' });
    }
  };
  ...
  const handleDownloadAllDocuments = async (entityType: string, entityId: number) => {
    try {
      ...
      setAlert({ message: 'Error downloading documents', severity: 'error' });
    }
  };
  ...
  return (
    <Box p={3}>
          ...
          <TableBody>
            {demandes.map((demande) => (
              <TableRow key={demande.id}>
                ...
                <TableCell>{demande.location}</TableCell>
                <TableCell>{new Date(demande.date).toLocaleString()}</TableCell>
                <TableCell>{demande.type}</TableCell>
                <TableCell>
                  <Chip
                    label={t(demande.status)}
                    color={
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth sx={{ zIndex: 1200 }}>
        <DialogTitle sx={{ fontWeight: 600 }}>{t('Demande Details')}</DialogTitle>
        <DialogContent sx={{ zIndex: 1200 }}>
          <Card variant="outlined" sx={{ p: 2, mb: 2, zIndex: 1400 }}>
            <CardContent>
              ...
              <Typography variant="body1" color="textSecondary">
                <strong>{t('Type')}:</strong> {selectedDemande?.type}
              </Typography>
              <Typography variant="body1" color="textSecondary">
                <strong>{t('Status')}:</strong> {selectedDemande?.status}
              </Typography>
              <Divider sx={{ my: 2 }} />
              <Typography variant="h6" fontWeight={600} gutterBottom>
                {t('Created By')}
